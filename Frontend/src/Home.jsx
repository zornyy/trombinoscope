...
export default function Home() {
  ...
  const getRecords = async() => {
    ...
    try {
      const fetched = await db.collection('Trombino').getFullList({
        sort: '-created'
      });
      setRecords(fetched)
                ...
                <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
                    <ul className="space-y-2 font-medium">
                        ...
                        </li>
                        {
                          records.map((item, i) =>
                            <li key={i}>
                                <a href="#" onClick={() => setTromId(item.id)} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group focus:bg-gray-700">
